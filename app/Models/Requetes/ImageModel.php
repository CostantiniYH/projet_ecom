...
//     try {
//         $sql = "SELECT $table.*, c.nom AS nom_categorie 
//         FROM $table
//         INNER JOIN t_categories c ON $table.id_categorie = c.id 
//         WHERE $table.$champ = ?";
...
function findImageBy2 ($pdo, $id) {
    try {
        $sql = "SELECT i.nom, i.image, i.id_categorie, c.nom AS nom_categorie  
        FROM t_images i 
        INNER JOIN t_categories c ON i.id_categorie = c.id
        WHERE i.id_user = ?";
